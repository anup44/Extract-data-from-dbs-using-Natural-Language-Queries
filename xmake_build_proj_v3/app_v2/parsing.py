...
class Grammar:
    def __init__(self, preprocessor, rules=[], 
                annotators=[], taggers=[], start_symbol='$ROOT'):
        self.categories = set()
...
def parse_input(grammar, input):
    ...
    # input = preprocess(input).initial_preprocess()
    input = grammar.preprocessor.initial_preprocess(input)
    input = grammar.preprocessor.deeppavlov_spelling(input)
    tokens = input.split()
    chart = defaultdict(list)
    indexed_chart = defaultdict(lambda: defaultdict(list))
    for tagger in grammar.taggers:
        tagger.annotate_query(input, chart)
    for j in range(1, len(tokens) + 1):
        for i in range(j - 1, -1, -1):
            apply_annotators(grammar, chart, tokens, i, j)               # new!
            apply_lexical_rules(grammar, chart, tokens, i, j)
            apply_binary_rules(grammar, chart, indexed_chart, i, j)
...
def apply_lexical_rules(grammar, chart, tokens, i, j):
    ...
    lower_tokens = tuple(t.lower() for t in tokens[i:j])
    # for rule in grammar.lexical_rules[tuple(tokens[i:j])]:
    for rule in grammar.lexical_rules[lower_tokens]:
        if not check_capacity(chart, i, j):
            return
        # chart[(i, j)].append(Parse(rule, tokens[i:j]))
        chart[(i, j)].append(Parse(rule, lower_tokens))

def apply_binary_rules_old(grammar, chart, i, j):
...
def index_parses(chart, indexed_chart, i, j):
    if not indexed_chart[(i, j)]:
        # parses_index = defaultdict(list)
        # parse_list = chart[(i, j)]
        for p in chart[(i, j)]:
            indexed_chart[(i, j)][p.rule.lhs].append(p)
        # indexed_chart[(i, j)] = parses_index
    return indexed_chart[(i, j)]

def apply_binary_rules(grammar, chart, indexed_chart, i, j):
    """Add parses to chart cell (i, j) by applying binary rules."""
    for k in range(i + 1, j):
        parses_0 = index_parses(chart, indexed_chart, i, k)
        parses_1 = index_parses(chart, indexed_chart, k, j)
        for rhs in grammar.binary_rules.keys():
            if rhs[0] in parses_0 and rhs[1] in parses_1:
                for rule in grammar.binary_rules[rhs]:
                    for parse1, parse2 in product(parses_0[rhs[0]], parses_1[rhs[1]]):
                        if not check_capacity(chart, i, j):
                            return
...
def write_chart(chart, file_name):
    """Print the chart.  Useful for debugging."""
    spans = sorted(list(chart.keys()), key=(lambda span: span[0]))
    spans = sorted(spans, key=(lambda span: span[1] - span[0]))
    f = open(file_name, 'w')
    for span in spans:
        if len(chart[span]) > 0:
            f.write('%-12s' % str(span) + ' ')
            f.write(str(chart[span][0]) + '\n')
            for entry in chart[span][1:]:
                f.write('%-12s' % ' ' + str(entry) + '\n')
